...
Contains all NN modules to be used in this sandbox.
...
# Factory methods used for building/initializing modules
# Mainly, these factories 'prefill' keyword args, and return the partial function for future use.
...
def _build_modules(factories, values, args=None, none_identity=True, key_id='type', extra_kwargs=None):
    ...
    for i in range(len(values)):
        mod = _build_module(
            factories,
            values[i],
            *(args[i] if args is not None else tuple()),
            none_identity=False,
            key_id=key_id,
            extra_kwargs=extra_kwargs[i] if extra_kwargs is not None else None
        )
        modules.append(mod)
...
# For parsing dimensions of a NN from a config file (i.e. - YAML)
def parse_dims(dims):
...
def cnn(in_dim, in_res, out_dim, channels, kernels, strides, paddings, norm,
            act, init, out_bias, out_norm, *, memory_format=None, device=None):
...
def transpose_cnn(in_dim, out_dim, in_res, channels, kernels, strides, paddings, norm, act,
            init, *, memory_format=None, device=None):
    channels, kernels, strides, paddings = [parse_dims(x) for x in (channels, kernels, strides, paddings)]
    num_layers = len(channels)
    if len(kernels) != num_layers or len(strides) != num_layers or len(paddings) != num_layers:
        raise ValueError("Number of channels, strides, kernels, and paddings must be equal.")
...
class LayerNorm(nn.LayerNorm):
    """ Channel-wise layer norm for 2d inputs """
...
class RandomIntensity(nn.Module):
    """ Adds random intensity to pixels. """
...
class RandomShift(nn.Module):
    """ Provides small pixel shifts to orignal image """
...
# Activations, including custom ones
_acts = {
    'none': lambda: None,
    'relu': lambda: nn.ReLU(),
    'silu': lambda: nn.SiLU(),
...
# Normalization (1D), including custom ones
_norms_1d = {
...
# Normalization (2D), including custom ones
_norms_2d = {
    'none': lambda d, device=None: None,
...
# Initialization, including custom ones
_inits = {
...
# Transforms, including custom ones
_transforms = {
    'none': lambda: None,
...
class VectorMLP(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, dims, norm, act, init, out_bias, out_norm, objective, device=None):
        super().__init__()
...
class Optimizer:
    """
    Wrapper for optimizer which handles:
        1. learning rate scheduling
        2. gradient clipping
        3. gradient scaling (autocast)
    """
